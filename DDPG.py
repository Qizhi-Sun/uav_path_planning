...
class DDPG:
    def __init__(self, state_dim, hidden_dim, action_dim, UAV_num, action_bound, sigma, actor_lr, critic_lr, tau, gamma, device):
    ...
    def take_action(self, state):
        ...
        action1 = self.actor(state_1).detach().cpu().numpy().squeeze(0)
        action2 = self.actor_1(state_2).detach().cpu().numpy().squeeze(0)
        action3 = self.actor_2(state_3).detach().cpu().numpy().squeeze(0)
    ...
    def update(self, transition_dict):
        ...
        next_q_value = self.critic_target(next_states.reshape(64, 30), next_actions)
