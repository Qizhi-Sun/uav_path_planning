...
class DDPG:
    ...
    def update(self, transition_dict):
        ...
        next_actions = self.actor_target(next_states[:,0,:]).detach()
        ...
        critic_loss = torch.mean(F.mse_loss(q_value, q_targets))
        ...
        #     if param.grad is not None:
        #         print(f"actor layername : {name},  actor's grad is :{param.grad.norm()}")
        #     else:
        #         print("None")
        ...
        next_q_value_1 = self.critic_target_1(next_states.reshape(64, 30), next_actions_1).detach()
        q_targets_1 = rewards[:,1].view(64,1) + self.gamma * next_q_value_1 * (1 - dones)
        critic_loss_1 = torch.mean(F.mse_loss(q_targets_1, q_value_1))
        ...
        next_q_value_2 = self.critic_target_2(next_states.reshape(64, 30), next_actions_2).detach()
        q_targets_2 = rewards[:,2].view(64,1) + self.gamma * next_q_value_2 * (1 - dones)
        critic_loss_2 = torch.mean(F.mse_loss(q_value_2, q_targets_2))
